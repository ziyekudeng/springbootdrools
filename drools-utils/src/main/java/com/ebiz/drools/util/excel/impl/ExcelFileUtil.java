package com.ebiz.drools.util.excel.impl;

import com.ebiz.drools.util.CollectionUtil;
import com.ebiz.drools.util.StringUtil;
import com.ebiz.drools.util.excel.model.ExcelModel;
...
/**
 * 描述：
 * CLASSPATH: com.sinosoft.util.excel.ExcelFileUtil
 * VERSION:   1.0
 * Created by gaowei
 *
 */
public class ExcelFileUtil extends BaseExcelFile implements Serializable {
     ...
     * throws Exception
     */
    public static String downloadExcelFile2003Util(ExcelModel fileMod, String path) throws IOException {
        String mes = "";
        //判断参数是否为空
        if (null != fileMod) {

            //对数据进行验证和处理
            List<ExcelModel> modeList = checkOrSplitList(fileMod);
     ...
     * @return 新的 ExcelModel 集合
     */
    private static List<ExcelModel> checkOrSplitList(ExcelModel excelMapModel) {

        //存放分割后的model集合
        List<ExcelModel> modeList = new ArrayList<>();

        //excel2003版本一个工作表最多可有65536行
        if (excelMapModel.getList().size() > MAX_COL_SIZE) {
            //分割数据集合
            List<List<Object[]>> resultList = splitList(excelMapModel.getList(), MAX_COL_SIZE);
            //循环创建新的 ExcelModel
            for (int i = 0; i < resultList.size(); i++) {
                //创建新的 ExcelModel
                ExcelModel newExcelModel = new ExcelModel();
                //复制 ExcelModel 里的其他信息
                BeanUtils.copyProperties(excelMapModel, newExcelModel, "list");
                //设置新的list集合
                newExcelModel.setList(resultList.get(i));
                //设置新的sheet名称(默认 原名称拼接当前循环下表 例如：原sheet名称；测试  新名称：测试_1 )
                newExcelModel.setSheetName(excelMapModel.getSheetName() + "_" + i);

                //将新的 ExcelModel 添加到新的集合里
                modeList.add(newExcelModel);
            }
        } else {
     ...
     * throws Exception
     */
    public static String downBatchExcelFile2003Util(List<ExcelModel> fileMods, String path) throws IOException {
     ...
     * throws Exception
     */
    public static String downBatchExcelFile2003Util(List<ExcelModel> fileMods, String path, String fileName) throws IOException {
        // 文件路径
        ...
        try (HSSFWorkbook workbook = new HSSFWorkbook()) {
            ...
            if (!CollectionUtil.collectionIsNull(fileMods)) {
                ...
                for (int k = 0; k < fileMods.size(); k++) {
                    //获取对象ExcelModel
                    ExcelModel excelMapModel = fileMods.get(k);
                    //检查当前对象里的数据是否超过 EXCEL 2003 最大行数,如果超过，进行分割
                    List<ExcelModel> modeList = checkOrSplitList(excelMapModel);
                    //循环ExcelModel
                    for (ExcelModel fileMod : modeList) {
                        //创建excel
     ...
     */
    private static void createExcel2003(HSSFWorkbook workbook, int index, ExcelModel fileMod) {
        ...
        if (null != fileMod.getHeaders()) {
            ...
            throw new NullPointerException("传入的参数ExcelModel对象中的集合属性为空，参数传入错误，不能生成Excel！");
        }
    }
     ...
     */
    private static void createRow(ExcelModel fileMod, HSSFSheet sheet, HSSFCellStyle style, HSSFCellStyle textStytle) {
        ...
        createRowValue(fileMod, sheet, textStytle, colSize);
     ...
     * @param colSize    列
     */
    private static void createRowValue(ExcelModel fileMod, HSSFSheet sheet, HSSFCellStyle textStytle, int colSize) {
        HSSFRow row;
        //判断是否有要写入的数据
        if (!CollectionUtil.collectionIsNull(fileMod.getList())) {
            //创建excel行和内容
            //获取数据总量
            int totalSize = fileMod.getList().size();
            //获取数据
            Object[] obj;
            //往Excel中写入数据
            for (int j = 0; j < totalSize; j++) {
                //获取数据
                obj = fileMod.getList().get(j);
                //创建行
                ...
                for (int n = 0; n < colSize; n++) {
                    //获取单元格数据
                    String value = null != obj[n] ? obj[n].toString() : "";
                    //创建第j个单元格
     ...
     * throws Exception
     */
    public static String downloadExcelFile2007Util(ExcelModel fileMod, String path) throws IOException {
     ...
     * throws Exception
     */
    public static String downBatchExcelFile2007Util(List<ExcelModel> fileMods, String path) throws IOException {
     ...
     * throws Exception
     */
    public static String downBatchExcelFile2007Util(List<ExcelModel> fileMods, String path, String fileName) throws IOException {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            if (!CollectionUtil.collectionIsNull(fileMods)) {
                ...
                for (int k = 0; k < fileMods.size(); k++) {
                    //获取对象ExcelModel
                    ExcelModel fileMod = fileMods.get(k);
                    //判断fileMod.getList()集合是否为空
                    if (null != fileMod.getHeaders()) {
                        //创建excel
     ...
     * @param fileMod  excel信息
     */
    private static void createExcel(XSSFWorkbook workbook, int index, ExcelModel fileMod) {
        //表名默认为 “sheet”+下表
        String sheetName = null == fileMod.getSheetName() ? "sheet" + index : fileMod.getSheetName();
        // 添加工作表
        XSSFSheet sheet = workbook.createSheet(sheetName);
        //创建excel信息
        XSSFCellStyle style = createExcel(workbook);
        //生成excel内容格式  (防止科学技术法)
     ...
     */
    private static XSSFCellStyle createExcel(XSSFWorkbook workbook) {
        // 生成列标题样式
     ...
     * @param textStytle 内容样式
     */
    private static void createRow(ExcelModel fileMod, XSSFSheet sheet, XSSFCellStyle style, XSSFCellStyle
            textStytle) {
        // 创建表格标题行
     ...
     * @param colSize    列
     */
    private static void createRowValue(ExcelModel fileMod, XSSFSheet sheet, XSSFCellStyle textStytle, int colSize) {
        XSSFRow row;
        //判断是否有要写入的数据
        if (!CollectionUtil.collectionIsNull(fileMod.getList())) {
            //获取数据总量
