...
-- 导出 lli_drools 的数据库结构
CREATE DATABASE IF NOT EXISTS `lli_drools` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `lli_drools`;

-- 导出  表 lli_drools.RULE_ACTION_INFO 结构
...
-- 正在导出表  lli_drools.RULE_ACTION_PARAM_INFO 的数据：~2 rows (大约)
...
-- 导出  表 lli_drools.RULE_ACTION_PARAM_VALUE_INFO 结构
...
-- 正在导出表  lli_drools.RULE_ACTION_PARAM_VALUE_INFO 的数据：~2 rows (大约)
...
-- 导出  表 lli_drools.RULE_ACTION_RULE_REL 结构
...
-- 正在导出表  lli_drools.RULE_ACTION_RULE_REL 的数据：~2 rows (大约)
...
-- 导出  表 lli_drools.RULE_CONDITION_INFO 结构
...
-- 正在导出表  lli_drools.RULE_CONDITION_INFO 的数据：~1 rows (大约)
...
-- 导出  表 lli_drools.RULE_ENTITY_INFO 结构
...
-- 正在导出表  lli_drools.RULE_ENTITY_ITEM_INFO 的数据：~3 rows (大约)
...
-- 正在导出表  lli_drools.RULE_INFO 的数据：~1 rows (大约)
...
-- 导出  表 lli_drools.RULE_PROPERTY_INFO 结构
...
-- 正在导出表  lli_drools.RULE_PROPERTY_INFO 的数据：~12 rows (大约)
...
-- 导出  表 lli_drools.RULE_PROPERTY_REL 结构
...
-- 正在导出表  lli_drools.RULE_PROPERTY_REL 的数据：~3 rows (大约)
...
-- 导出  表 lli_drools.RULE_SCENE_ENTITY_REL 结构
...
-- 正在导出表  lli_drools.RULE_SCENE_ENTITY_REL 的数据：~1 rows (大约)
...
-- 导出  表 lli_drools.RULE_SCENE_INFO 结构
...
-- 正在导出表  lli_drools.RULE_SCENE_INFO 的数据：~1 rows (大约)
...
INSERT INTO `RULE_SCENE_INFO` (`SCENE_ID`, `SCENE_IDENTIFY`, `SCENE_TYPE`, `SCENE_NAME`, `SCENE_DESC`, `IS_EFFECT`, `CRE_USER_ID`, `CRE_TIME`, `REMARK`) VALUES
	(1, 'test', 1, '测试', '测试规则引擎', 1, 1, '2017-07-20 16:56:10', '测试');
/*!40000 ALTER TABLE `RULE_SCENE_INFO` ENABLE KEYS */;

-- 导出  表 lli_drools.RULE_VARIABLE 结构
...
-- 正在导出表  lli_drools.RULE_VARIABLE 的数据：~2 rows (大约)
