...
import com.ebiz.drools.exception.DroolsExceptionEnum;
import com.ebiz.drools.exception.ServiceException;
import com.ebiz.drools.model.*;
import com.ebiz.drools.model.fact.RuleExecutionObject;
import com.ebiz.drools.service.*;
import com.ebiz.drools.util.CollectionUtil;
import com.ebiz.drools.util.drool.DroolsUtil;
import com.ebiz.drools.util.drool.RuleUtils;
import com.ebiz.drools.util.spring.SpringContextUtil;
...
public class DroolsRuleEngineServiceImpl implements DroolsRuleEngineService {
    ...
    private RuleExecutionObject compileRule(RuleExecutionObject ruleExecutionObject, final String scene) {
        ...
        // 2.引入global全局对象
        // 暂时只设定一个
        droolRuleStr.append("import RuleExecutionResult").append(";").append(LINE_SEPARATOR);
        droolRuleStr.append("global RuleExecutionResult _result").append(";").append(LINE_SEPARATOR);
        //将 RuleExecutionObject 也引入进来
        droolRuleStr.append("import RuleExecutionObject").append(";").append(LINE_SEPARATOR);
    ...
    private void insertImportInfo(StringBuilder droolRuleStr, List<BaseRuleEntityInfo> entityList,
        ...
        if (!CollectionUtil.collectionIsNull(actionList)) {
            ...
            //如果有实现类，则把实现类接口拼装进去
            if (implFlag) {
                droolRuleStr.append(IMPORT).append(" ").append("DroolsActionService").append(";").append(LINE_SEPARATOR);
            }
        }
    }
}
