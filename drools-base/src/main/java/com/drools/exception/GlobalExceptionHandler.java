...
import com.drools.util.restful.ResultResponseUtils;
...
import static com.drools.util.restful.ResultResponseUtils.error;
...
public class GlobalExceptionHandler implements ErrorController {
    ...
    public ResponseResult<Object> handle(Exception e) {
        ...
        if (e instanceof HttpRequestMethodNotSupportedException) {
            return ResultResponseUtils.error(ExceptionEnum.SYS_METHOD_NOT_ALLOWED);
        }
        if (e instanceof HttpMessageNotReadableException) {
            return ResultResponseUtils.error(ExceptionEnum.SYS_JSON_DATA_ERROR);
        }
        if (e instanceof HttpMediaTypeNotSupportedException) {
            return ResultResponseUtils.error(ExceptionEnum.SYS_UNSUPPORTED_MEDIA_TYPE);
        }
        if (e instanceof SQLSyntaxErrorException) {
            return ResultResponseUtils.error(ExceptionEnum.SYS_DATABASE_FIELD_NOT_EXIST);
        }
        if (e instanceof DataAccessException) {
            return ResultResponseUtils.error(ExceptionEnum.SYS_DATABASE_FAILURE);
        }

        return ResultResponseUtils.error(ExceptionEnum.SYS_FAILURE_EXCEPTION);
    }
    ...
    public ResponseResult<Object> handle(HttpServletResponse response) {
        switch (HttpStatus.valueOf(response.getStatus())) {
            case NOT_FOUND:
                return ResultResponseUtils.error(ExceptionEnum.SYS_SERVICE_NOT_FOUND_ERROR);
            default:
                return ResultResponseUtils.error(ExceptionEnum.SYS_FAILURE_EXCEPTION);
        }
    }
